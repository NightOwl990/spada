...
public class PopupFilterMenu extends LinearLayout {
    ...
    private PopUpFilterTabClickListener listener;
    private DisplayMetrics metrics;
    int lengthHeight = getResources().getDimensionPixelSize(R.dimen.length_40);
    int lengthTextMargin =  getResources().getDimensionPixelSize(R.dimen.length_4);

    public PopupFilterMenu(Context context) {
        this(context, (AttributeSet) null);
    }

    public PopupFilterMenu(Context context, PopUpFilterTabClickListener listener) {
        this(context, (AttributeSet) null);
        this.listener = listener;
    }

    public void setListener(PopUpFilterTabClickListener listener) {
        this.listener = listener;
    }
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        LinearLayout.LayoutParams params = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, lengthHeight);
        ...
        line.setLayoutParams(new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp2Px(1f)));
    ...
    public void closeMenu() {
        if (current_tab_position != -1) {
            LinearLayout lnTab = (LinearLayout) tabMenuView.getChildAt(current_tab_position);
            TextView tabText = (TextView) lnTab.getChildAt(0);
    ...
    public void setTabText(String text) {
        if (current_tab_position != -1) {
            LinearLayout lnCurrentTab = (LinearLayout) tabMenuView.getChildAt(current_tab_position);
            ((TextView) lnCurrentTab.getChildAt(0)).setText(text);
        }
    }


    public void setTabTextPosition(int position, String text) {
            LinearLayout lnCurrentTab = (LinearLayout) tabMenuView.getChildAt(position*2);
            ((TextView) lnCurrentTab.getChildAt(0)).setText(text);
    ...
    private void addTabs(List<String> tabTexts, int i) {
        ...
        lnLayout.setLayoutParams(new LayoutParams(0, lengthHeight, 1));
        ...
        tab.setLayoutParams(new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, lengthHeight));
        tab.setCompoundDrawablesWithIntrinsicBounds(null, null, getResources().getDrawable(menuUnselectedIcon), null);
        tab.setCompoundDrawablePadding(lengthTextMargin);
//        LinearLayout.LayoutParams params = (LinearLayout.LayoutParams)tab.getLayoutParams();
//        params.setMargins(0, 0, lengthTextMargin, 0);

//        tab.setPadding(5, 0, lengthTextMargin, 0);
        tab.setText(tabTexts.get(i));
        tab.setTextColor(textUnselectedColor);
        lnLayout.addView(tab);
        lnLayout.setOnClickListener(v -> switchMenu(lnLayout));
        tabMenuView.addView(lnLayout);
    ...
    private void switchMenu(LinearLayout willSelectTab) {
        for (int i = 0; i < tabMenuView.getChildCount(); i += 2) {
            if (willSelectTab == tabMenuView.getChildAt(i)) {//选中的tab
                if (current_tab_position == i) {
                    listener.onTabClick(willSelectTab, i, false);
                    closeMenu();
                } else {
                    listener.onTabClick(willSelectTab, i, true);
                    ...
                    LinearLayout lnTab = (LinearLayout) tabMenuView.getChildAt(i);
                    TextView tabText = (TextView) lnTab.getChildAt(0);
